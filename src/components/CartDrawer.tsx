...
export default function CartDrawer() {
  ...
  // Keep the drawer mounted briefly after `isOpen` flips false so the
  // exit transition runs to completion before we unmount the tree.
  ...
  return (
    <>
      ...
      <div
        onClick={closeCart}
        className={`fixed inset-0 z-[60] bg-bourbon-deep/40 backdrop-blur-sm transition-opacity duration-300 ${
      ...
      {/* Drawer */}
      <div
        className={`fixed top-0 right-0 bottom-0 z-[70] w-full max-w-md bg-bourbon-cream border-l border-bourbon-deep/10 shadow-2xl flex flex-col transition-transform duration-300 ease-out ${
          ...
          {items.length === 0 ? (
            ...
            <div className="space-y-4">
              {items.map((item) => (
                <div
                  key={item.id}
                  className="animate-fade-in flex gap-3 sm:gap-4 bg-white p-3 border border-bourbon-deep/10"
                >
                  {/* Image */}
